...
public class ScoreMongoDAOImpl implements ScoreMongoDAO {
	...
	@Override
	public List<ScoreDTO> findIntCriteria(String key, int value) {
		Criteria criteria = new Criteria(key);
		
		criteria.is(value);
		Query query = new Query(criteria);
		List<ScoreDTO> documents = mongoTemplate.find(query, ScoreDTO.class, "score");
	...
	public List<ScoreDTO> findCriteria(String key, String value) {
		...
		//Query 클래스의 addCriteria 메소드 & Criteria 클래스의 where 메소드 활용
	...
	public ScoreDTO findById(String key, String value) {
		...
		//4. MongoTemplate 클래스의 메소드를 호출 & Query 객체를 매개변수로 전달
	...
	public void update(ScoreDTO document) {
		//수정할 document에 조건을 적용((RDBMS) where절)
		Criteria criteria = new Criteria("id");
		criteria.is(document.getId());
		Query query = new Query(criteria);
		
		//수정될 데이터를 셋팅((RDBMS) set절)
